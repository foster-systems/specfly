...
- **WHEN** a `push` targets `refs/heads/change/<name>`, its tip-commit subject starts
  with `@spec:apply`, and the sender is not `github-actions[bot]`
...
- **WHEN** a `push` targets `refs/heads/change/<name>`, the sender is `specfly[bot]`,
  and the tip-commit subject does not start with `@spec:apply`
...
with `@spec:apply` (case-sensitive); the sender is not `github-actions[bot]`; and
...
- **WHEN** the trigger subject is `@spec:apply model=opus effort=high foo=bar`
- **THEN** `model` is `opus`, `effort` is `high`, and `foo` is ignored

#### Scenario: No args yields empty options

- **WHEN** the trigger subject is exactly `@spec:apply`

...
the run row to `status="pr_opened"` with the `pr_number`.
...
- **AND** updates the run row to `status="pr_opened"` with the new `pr_number`
...
unconditionally without reading check state (no `Checks: read` permission), and the
run row SHALL be updated (`updated_at`, keeping `status="pr_opened"`).
...
- **AND** updates the run row's `updated_at` while keeping `status="pr_opened"`
...
dispatched run produces no result push (apply made no changes or failed). This
known-state SHALL be documented in the backend README; a TTL/cleanup job is out of scope.

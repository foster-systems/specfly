...
- **WHEN** a human pushes a non-`/sfx:apply` commit to `change/<name>` while a run
  is `dispatched`
...
with `/sfx:apply`. This push arrives as `specfly[bot]`, so it re-fires the adopter's
...
- **THEN** its subject does not start with `/sfx:apply` and its sender is

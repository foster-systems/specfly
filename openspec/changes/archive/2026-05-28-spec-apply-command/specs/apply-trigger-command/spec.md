...
The system SHALL provide a local command `/spec:apply` that crafts a conformant
...
and optional `model=<m>` / `effort=<e>` arguments (`/spec:apply [<name>] [model=<m>]
[effort=<e>]`). It SHALL be distinct from `/opsx:apply`, which implements a change's
tasks locally; `/spec:apply` triggers the remote apply run instead.

#### Scenario: Command crafts and pushes a trigger

- **WHEN** a user runs `/spec:apply <name>` on a repo set up for Specfly
- **THEN** the command creates a `/spec:apply <name>` tip commit on `change/<name>`
...
- **THEN** they use `/spec:apply` (remote trigger), not `/opsx:apply` (local implement)
...
- **WHEN** the user runs `/spec:apply add-auth`
...
its first line SHALL be `/spec:apply <name>`, optionally followed by `model=<m>` and
...
NOT begin with `/spec:apply` (so it is not itself a trigger), then create the trigger
commit. The trigger commit SHALL be created with `--allow-empty`.

#### Scenario: Trigger subject carries optional args

- **WHEN** the user runs `/spec:apply add-auth model=opus effort=high`
- **THEN** the trigger commit's first line is `/spec:apply add-auth model=opus effort=high`

#### Scenario: Pending edits committed separately

- **WHEN** the working tree has uncommitted edits under `openspec/changes/<name>/`
- **THEN** those edits are committed under a non-`/spec:apply` subject before the
...
- **WHEN** the user runs `/spec:apply <name>` a second time with no new edits
...
- **THEN** the command tells the user the dispatch/PR flow, where to watch it, and
  that re-running is simply `/spec:apply` again
...
trigger contract (`change/<name>` branch + `/spec:apply` first-line tip commit +
